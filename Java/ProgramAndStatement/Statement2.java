...
public class Statement2 extends StatementSecondary {
    ...
    public final void addToBlock(int pos, Statement s) {
        ...
        // Cast the statement to the correct type
        Statement2 state2 = (Statement2) s;

        // Add the statement's tree representation to the block at the specified position
        this.rep.addSubtree(pos, state2.rep);
    ...
    public final Statement removeFromBlock(int pos) {
        ...
        // Create a new instance of the statement to hold the removed subtree
        Statement2 s = this.newInstance();

        // Remove the subtree at the specified position and assign it to the new instance
        s.rep = this.rep.removeSubtree(pos);

        // Return the new instance containing the removed statement
        return s;
    }
    ...
    public final void assembleIfElse(Condition c, Statement s1, Statement s2) {
        ...
        Statement2 state2S1 = (Statement2) s1;
        Statement2 state2S2 = (Statement2) s2;
        StatementLabel label = new StatementLabel(Kind.IF_ELSE, c);
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        children.add(0, state2S1.rep);
        children.add(1, state2S2.rep);
        this.rep.assemble(label, children);
        state2S1.createNewRep();
        state2S2.createNewRep();
    ...
    public final Condition disassembleIfElse(Statement s1, Statement s2) {
        ...
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        StatementLabel label = this.rep.disassemble(children);
        state2S2.rep = children.remove(1);
        state2S1.rep = children.remove(0);
        this.createNewRep(); // clears
        return label.condition;
    }
